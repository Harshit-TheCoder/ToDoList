...
// thursday To-Do
const todoList = document.querySelector('.thursday-todo-list');
const addButton = document.querySelector('.thursday-todo-button');
const taskInput = document.querySelector('.thursday-task-name');
const timeInput = document.querySelector('.thursday-task-time');
const descriptionInput = document.querySelector('.thursday-task-description');

// Load tasks from localStorage
const loadTasks = () => {
    const tasks = JSON.parse(localStorage.getItem('thursday-tasks')) || [];
    tasks.forEach(addTaskToDOM);
};

// Save tasks to localStorage
const saveTasks = (tasks) => {
    localStorage.setItem('thursday-tasks', JSON.stringify(tasks));
};
...
const addTaskToDOM = (taskObj) => {
    ...
    li.innerHTML = `
        ...
        <div class="col-4">
            <center><button class="thursday-delete-button btn btn-danger">Delete</button></center>
        </div>
    `;
    const deleteButton = li.querySelector('.thursday-delete-button');
...
const deleteTask = (taskObj, taskElement) => {
    let tasks = JSON.parse(localStorage.getItem('thursday-tasks')) || [];
    tasks = tasks.filter(
