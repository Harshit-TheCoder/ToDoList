...
// saturday To-Do
const todoList = document.querySelector('.saturday-todo-list');
const addButton = document.querySelector('.saturday-todo-button');
const taskInput = document.querySelector('.saturday-task-name');
const timeInput = document.querySelector('.saturday-task-time');
const descriptionInput = document.querySelector('.saturday-task-description');

// Load tasks from localStorage
const loadTasks = () => {
    const tasks = JSON.parse(localStorage.getItem('saturday-tasks')) || [];
    tasks.forEach(addTaskToDOM);
};

// Save tasks to localStorage
const saveTasks = (tasks) => {
    localStorage.setItem('saturday-tasks', JSON.stringify(tasks));
};
...
const addTaskToDOM = (taskObj) => {
    ...
    li.innerHTML = `
        ...
        <div class="col-4">
            <center><button class="saturday-delete-button btn btn-danger">Delete</button></center>
        </div>
    `;
    const deleteButton = li.querySelector('.saturday-delete-button');
...
const deleteTask = (taskObj, taskElement) => {
    let tasks = JSON.parse(localStorage.getItem('saturday-tasks')) || [];
    tasks = tasks.filter(
