...
// Monday To-Do
const todoList = document.querySelector('.monday-todo-list');
const addButton = document.querySelector('.monday-todo-button');
const taskInput = document.querySelector('.monday-task-name');
const timeInput = document.querySelector('.monday-task-time');
const descriptionInput = document.querySelector('.monday-task-description');

// Load tasks from localStorage
const loadTasks = () => {
    const tasks = JSON.parse(localStorage.getItem('monday-tasks')) || [];
    tasks.forEach(addTaskToDOM);
};

// Save tasks to localStorage
const saveTasks = (tasks) => {
    localStorage.setItem('monday-tasks', JSON.stringify(tasks));
};
...
const addTaskToDOM = (taskObj) => {
    ...
    li.innerHTML = `
        ...
        <div class="col-4">
            <center><button class="monday-delete-button btn btn-danger">Delete</button></center>
        </div>
    `;
    const deleteButton = li.querySelector('.monday-delete-button');
...
const deleteTask = (taskObj, taskElement) => {
    let tasks = JSON.parse(localStorage.getItem('monday-tasks')) || [];
    tasks = tasks.filter(
